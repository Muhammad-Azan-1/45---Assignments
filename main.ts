...
console.log(
  "Title case = ",
  str.replace(/\b\w/g, (c) => c.toLocaleUpperCase())
);
...
let fullName1: string = "PROPHET MUHAMMAD (ﷺ)";
...
let favorite_Transporte: string[] = [
  "Honda CB 150 F-SE",
  "Mercedes Benz car",
  "Aqua hybrid car",
];
...
for (let val of moreGuest) {
  console.log(
    `Dear ${val} our table size is increased so we are inviting more memebers.`
  );
}
...
for (let val of moreGuest) {
  console.log(
    `Assalamualikum dear respected ${val} I would like to invite you all to dinner,`
  );
}
...
for (let val of shrinkingGuest) {
  console.log(
    `Dear ${val} we are sorry our table size is decreasd so we can only invite two memebers.`
  );
}
...
console.log(
  "\n",
  " Original Arry to Reverse Alphabtic Order = ",
  favoritePlaces
);
...
function car_info(Manufacturer:string, make_model:string ,...moreInf:any[]){    //... gather items(Arguments passed in call function) into an arry
